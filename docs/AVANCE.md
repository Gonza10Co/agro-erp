...
**Tests:** 460 backend (53 suites) + 329 frontend, verdes 🟢 · ambos builds limpios.
...
- [ ] **SEGUNDAS (S2: venta)** — falta `calidad` en `OrdenCompraLinea`/`DespachoLinea`/`FacturaLinea`, la clave de agrupación de `despacho-lineas.ts` y el mapa de precios de `factura-core.ts` por (producto, calidad). Sin esto una segunda saldría al precio de la primera y colapsaría con ella en el remito.
