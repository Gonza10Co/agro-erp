...
describe('OcService', () => {
  ...
  it('confirmar lanza BadRequest con los errores de validación', async () => {
    prisma.ordenCompra.findUnique.mockResolvedValue({
      ...
      lineas: [{
        productoConfigurado: { referencia: { tallaMin: { valor: 34 }, tallaMax: { valor: 46 } } },
  ...
  it('confirmar pasa la OC a CONFIRMADA cuando es válida', async () => {
    ...
    prisma.ordenCompra.update.mockResolvedValue({ id: 1, estado: 'CONFIRMADA' });
    const r = await service.confirmar(1);
    expect(prisma.ordenCompra.update).toHaveBeenCalledWith({ where: { id: 1 }, data: { estado: 'CONFIRMADA' } });
